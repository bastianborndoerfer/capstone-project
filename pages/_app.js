...
export default function App({ Component, pageProps }) {
  ...
  //   "https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=bitcoin%2C%20etherium%2C%20binancecoin%2C%20ripple%2C%20cardano%2C%20dogecoin%2C%20solana%2C%20polkadot%2C%20matic-netzwork%2C%20shiba-inu&order=market_cap_desc&per_page=10&page=1&sparkline=false&locale=en&precision=5",
  //   fetcher
  // );
  const [coinsInfo, setCoinsInfo] = useState(data);
    ...
    if (likedCoin) {
      setCoinsInfo(
        coinsInfo.map((coinInfo) =>
          coinInfo.id === id
            ? { ...coinsInfo, isFavorite: !coinInfo.isFavorite }
            : coinInfo
        )
      );
    } else {
